...
*/
	...
	Calendar.prototype._update_day_prepare_sln_booking_editor = function() {
		...
		$(".day-event-item__calendar-day .sln-icon--plus-circle").off('click').on(
			"click",
		...
		$( ".booking_tool_item .sln-icon--trash" ).off('click').on('click', function(event) {
		    	event.preventDefault();
		...
		$( ".confirm-delete a.btn-ok" ).off('click').on('click', function(event) {
			event.preventDefault();
		...
		$(".confirm-delete .close").off('click').on("click", function(event) {
			$(event.target)
				.closest(".day-event")
				.height(function(index, height) {
					return height - 100;
		...
		$(".booking_tool_item .sln-icon--user-check").off('click').on("click", function(
			event
		) {
			event.preventDefault();
			if ($(this).closest(".sln-free-version").length) {
